...
type (
	...
	//	Payment(ctx context.Context, request WalletToAccountRequest) (WalletToAccountResponse, error)
	//}
	...
	//// 1. handling of NameCheckRequest, 2. Handling of Collection (Payment)
	...
	//// requests and a WalletToAccountFunc to handle all Payment requests
	//Client struct {
	//	*tigo.BaseClient
...
//)
//
//func (w WalletToAccountFunc) Payment(ctx context.Context, request WalletToAccountRequest) (WalletToAccountResponse, error) {
...
func (client *BigClient) HandleRequest(ctx context.Context, requestType RequestType) http.HandlerFunc {
	ctx, cancel := context.WithTimeout(ctx, client.Timeout)
	defer cancel()
	return func(writer http.ResponseWriter, request *http.Request) {
		switch requestType {
